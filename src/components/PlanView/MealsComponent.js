...
class Meals extends Component {
  ...
  // update date
  submitDate = (meal_id) => () => {
    this.props.dispatch({ type: 'ADD_DATE', payload: { newDate: this.state.startDate, meal_id } });
    ...
    <Card style={styles.card}>
      ...
      <CardActions>
        ...
        <Button
          ...
          onClick={this.submitDate(meal.planned_id)}
        >
          Assign
        </Button>
        <Button
          ...
          color="secondary"
          styles={styles.button}
          onClick={this.removeMeal(meal.planned_id)}
        >
          Remove
      </Button>
      </CardActions>
  ...
  render() {
    return (
      <div>
        <h3>My Meals</h3>
        {this.state.plannedMeals.map(this.renderMealCard)}
      </div>
    );
  }
}
